...
function renderLanding(data) {
  ...
  const carouselHtml = carouselPosts
    .map((post, idx) => {
      const formattedDate = dayjs(post.date).format('DD/MM/YYYY');
      const image = post.image || '/imagens/post-placeholder.jpg';
  ...
  const projectsHtml = data.projects
    .map(project => {
      const image = project.image || '/imagens/project-placeholder.jpg';
      return `
        <article class="card">
  ...
  const productsHtml = data.products
    .map(product => {
      const image = product.image || '/imagens/product-placeholder.jpg';
      return `
